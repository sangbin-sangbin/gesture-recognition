...
class HandTracker:
    ...
    def lm_render(self, frame, region):
        if region.lm_score > self.lm_score_threshold:
            ...
            if self.state == 'break':
                cv2.putText(frame, "Break.. Next gesture is \"" + self.gestures[self.gesture_num] + "\"", (frame.shape[1] // 2 - 430, frame.shape[0] // 2 - 220), cv2.FONT_HERSHEY_SIMPLEX, 2, (255, 0, 0), 3)
            else:
                cv2.putText(frame, "Recorging \"" + self.gestures[self.gesture_num] + "\"", (frame.shape[1] // 2 - 430, frame.shape[0] // 2 - 220), cv2.FONT_HERSHEY_SIMPLEX, 2, (255, 0, 0), 3)
